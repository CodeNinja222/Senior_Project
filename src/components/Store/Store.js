import React, { useState, useEffect } from 'react';
import {Dialog, AppBar,Toolbar,IconButton,Typography,ToggleButtonGroup,ToggleButton,Box,Paper,Table,TableBody,TableCell,TableContainer,TableHead,TableRow,Chip,Card,CardContent,Avatar,
Slide,
  Stack
} from '@mui/material';
import { Rating,useTheme} from '@mui/material';
import {Close,LocalShipping,Dashboard,Star} from '@mui/icons-material';
import PaymentsIcon from '@mui/icons-material/Payments';
import ReviewsIcon from '@mui/icons-material/Reviews';
import FastfoodIcon from '@mui/icons-material/Fastfood';
import axios from 'axios';
import ProductionQuantityLimitsIcon from '@mui/icons-material/ProductionQuantityLimits';

const SlideFromRight = (props, ref) => (
  <Slide direction="left" ref={ref} {...props} timeout={2000}  />
);

const Store = ({ open, onClose,company }) => {
   //{companyID:"", companyType:"", name:""}
     const theme = useTheme();
  const [viewMode, setViewMode] = useState('overview');
  
 const [mainState,setmainState]=useState({items:[],orders:[],reviews:[],payments:[]})
  const handleViewModeChange = (_, newViewMode) => {
    if (newViewMode) {
      setViewMode(newViewMode);
    }
  };
 useEffect(()=>{
  const fetchAll=async()=>{
   const [items,orders,reviews,payments]=await Promise.all([
   await axios.post("http://127.0.0.1:5000/item/getItemCompany",{companyID:company.companyID}),
   await axios.post("http://127.0.0.1:5000/order/SelectAllOrderforDrivers"),
   await axios.post("http://127.0.0.1:5000/order/SelectReviews"),
   await axios.post("http://127.0.0.1:5000/order/AllPaymentAndTranscations")
   ])
   if(items&&orders&&reviews&&payments){
    
    const Orderincompany=orders.data.filter((it)=>it.company.company_id===company.companyID)
   const Reviewincompany=[]
   const Paymentsincompany=[]
   for(let order of Orderincompany){
     const found=reviews.data.find((re)=>re.reviewType=="order"&&re.orderID==order.orderID)
     if(found){
       Reviewincompany.push({...found,customername:order.customername})
     }
   }
   let pay=[...payments.data]
   for(let order of Orderincompany){
    while(pay.some((re)=>(re.Company_name==="Admin"&&re.orderID==order.orderID&&re.realamount>0.4)||(re.Company_name===company.name&&re.orderID==order.orderID))){
      const found=pay.find((re)=>(re.Company_name==="Admin"&&re.orderID==order.orderID&&re.realamount>0.4)||(re.Company_name===company.name&&re.orderID==order.orderID))
     if(found){
       Paymentsincompany.push(found)
       const index=pay.indexOf(found)
      pay.splice(index,1)
     }
    }
    
   }
   setmainState({items:items.data,orders:Orderincompany,reviews:Reviewincompany,payments:Paymentsincompany})
   }
   
  }
 fetchAll()
 },[company.companyID,company.name])
const computeRevenue=()=>{
  let total=0
  if(mainState.payments.length>0){
   for(let pay of mainState.payments.filter(pay=>pay.Company_name==company.name)){
     total+=pay.realamount
   }
   return total
  }
  return total
}
const computeAverageRating=()=>{
  let avg=0
  if(mainState.reviews.length>0){
   for(let re of mainState.reviews){
     avg+=re.rating
   }
   return avg/mainState.reviews.length
  }
  return avg
}
const displayOrderDetails=(items)=>{
  let arr=[]
  for(let item of items){
   arr.push(item.quantity+"x"+item.name)
  }
  return arr.join(",")
}
const calculateTotalAdminEarned=()=>{
  let total=0
  if(mainState.payments.length>0){
   for(let pay of mainState.payments.filter(pay=>pay.Company_name=="Admin")){
     total+=pay.realamount
   }
   return total
  }
  return total
}

  return (
    <Dialog
      open={open}
      onClose={() => onClose(false)}
      TransitionComponent={SlideFromRight}
      keepMounted
      fullScreen
      PaperProps={{
        sx: {
          m: 0,
          height: '100%',
          transform: 'translateX(0)',
          overflow: 'hidden',
          borderRadius: 0
        }
      }}
    >
      <AppBar position="static" sx={{ bgcolor: 'primary.main' }}>
        <Toolbar>
          <Typography variant="h6" sx={{ flexGrow: 1, color: 'white' }}>
           {company.name} Details
          </Typography>
          <IconButton color="inherit" onClick={() => onClose(false)}>
            <Close />
          </IconButton>
        </Toolbar>
      </AppBar>

      <Box sx={{ p: 3, backgroundColor: 'white', height: '100%', overflowY: 'auto' }}>
        <ToggleButtonGroup
          value={viewMode}
          exclusive
          onChange={handleViewModeChange}
          sx={{ mb: 3, '& .MuiToggleButton-root': { color: 'primary.main' } }}
        >
          <ToggleButton value="overview"><Dashboard sx={{ mr: 1 }} />Overview</ToggleButton>
          <ToggleButton value="Items"><FastfoodIcon sx={{ mr: 1 }}/>Items</ToggleButton>
          <ToggleButton value="Orders"><LocalShipping sx={{ mr: 1 }} />Orders</ToggleButton>
          <ToggleButton value="Payments"><PaymentsIcon sx={{ mr: 1 }} />Payments</ToggleButton>
          <ToggleButton value="Reviews"><ReviewsIcon sx={{ mr: 1 }} />Reviews</ToggleButton>
        </ToggleButtonGroup>

        {/* Overview */}
        {viewMode === 'overview' && (
          <Box>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3 }}>
              <Typography variant="h5">Company Overview</Typography>
              
            </Box>
            <Box sx={{ display: 'flex', gap: 3, flexWrap: 'wrap', mb: 3 ,mt:10}}>
              {[
                { label: 'Total Orders', value: mainState.orders.length },
                { label: 'Total number of items', value: mainState.items.length },
                { label: 'Revenue', value: `$${computeRevenue()?.toFixed(2)}` },
                { label: 'Avg. Rating', value: computeAverageRating()?.toFixed(1), icon: <Star color="primary" sx={{ ml: 1 }} /> }
              ].map((item, idx) => (
                <Card key={idx} sx={{ flexGrow: 1, minWidth: 200, borderLeft: '4px solid', borderColor: 'primary.main',minHeight:200 }}>
                  <CardContent style={{padding:80}}>
                    <Typography color="text.secondary" style={{fontSize:"20px"}}>{item.label}</Typography>
                    <Box sx={{ display: 'flex', alignItems: 'center',fontSize:"25px" }}>
                      <Typography variant="h4" color="primary.main">{item.value}</Typography>
                      {item.icon}
                    </Box>
                  </CardContent>
                </Card>
              ))}
            </Box>
            
          </Box>
        )}

        {/* Orders */}
        {viewMode === 'Orders' && (
          <Box>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3 }}>
              <Typography variant="h5" color="primary">Recent Orders</Typography>
              
            </Box>
            <TableContainer component={Paper} sx={{ borderLeft: '4px solid', borderColor: 'primary.main' }}>
              <Table>
                <TableHead sx={{ backgroundColor: 'primary.light' }}>
                  <TableRow>
                    <TableCell sx={{ color: 'white' }}>Order ID</TableCell>
                    <TableCell sx={{ color: 'white' }}>Customer</TableCell>
                    <TableCell sx={{ color: 'white' }}>Status</TableCell>
                    <TableCell sx={{ color: 'white' }}>Total</TableCell>
                    <TableCell sx={{ color: 'white' }}>Date & Time</TableCell>
                    <TableCell sx={{ color: 'white' }}>Details</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {mainState.orders.length>0?mainState.orders.map(order => (
                    <TableRow key={order.orderID} hover sx={{ cursor: 'pointer' }}>
                      <TableCell>#ORD-{order.orderID}</TableCell>
                      <TableCell>{order.customername}</TableCell>
                      <TableCell>
                        
                        <Chip
                          label={order.status}
                          color={
                            order.status === 'Pending' || order.status === 'OnProgress' ? 'primary' :
                            order.status === 'Completed' ? 'default' : 'error'
                          }
                          size="small"
                        />
                      </TableCell>
                      <TableCell>${order.totalAmount?.toFixed(2)}</TableCell>
                      <TableCell>{order.orderDate}</TableCell>
                      <TableCell>{displayOrderDetails(order?.Item)}</TableCell>
                    </TableRow>
                  )):<Typography variant='h6'> no current Orders found in this company</Typography>}
                </TableBody>
              </Table>
            </TableContainer>
          </Box>
        )}

       
        {viewMode === 'Items' && (
          <Box>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3 }}>
              <Typography variant="h5" color="primary">Items</Typography>
              
            </Box>
            <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 3 }}>
              {mainState.items.length>0?mainState.items.map(item => (
                <Card
                  key={item.itemID}
                  sx={{
                    borderLeft: '4px solid',
                    borderColor: item.stock >0? 'primary.main' : 'error.main',
                    transition: 'transform 0.2s',
                    '&:hover': { transform: 'scale(1.02)' }
                  }}
                >
                  <CardContent sx={{ display: 'flex', gap: 2 }}>
                    <Avatar src={item.url} alt={item.name}/>
                    <Box>
                      <Typography variant="h6">{item.name}</Typography>
                      <Typography variant="body2" color="text.secondary">${item.price}</Typography>
                      <Box sx={{ display: 'flex', mt: 1, gap: 2 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                          <ProductionQuantityLimitsIcon color="primary" fontSize="small" />
                          <Typography variant="body2" sx={{ ml: 0.5 }}>{item.stock}</Typography>
                        </Box>
                        
                      </Box>
                    </Box>
                  </CardContent>
                </Card>
              )):<Typography variant='h6'> no current items found in this company</Typography>}
            </Box>
          </Box>
        )}

       
        {viewMode === 'Payments' && (
          <Box>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3 }}>
              <Typography variant="h5" color="primary">Payments</Typography>
              <Stack spacing={1}>
                 <Chip
                          label={`Your Earned:$${calculateTotalAdminEarned()?.toFixed(2)}`}
                          color={"primary"}
                          size="small"
                        />
                  <Chip
                          label={`Total ${company.name} Earned:$${computeRevenue()?.toFixed(2)}`}
                          color={"primary"}
                          size="small"
                        />
               
              </Stack>
              
            </Box>
            <TableContainer component={Paper} sx={{ borderLeft: '4px solid', borderColor: 'primary.main' }}>
              <Table>
                <TableHead sx={{ backgroundColor: 'primary.light' }}>
                  <TableRow>
                    <TableCell sx={{ color: 'white' }}>PaymentID</TableCell>
                    <TableCell sx={{ color: 'white' }}>OrderID</TableCell>
                    <TableCell sx={{ color: 'white' }}>Total with delivery fees</TableCell>
                    <TableCell sx={{ color: 'white' }}>Payment Method</TableCell>
                    <TableCell sx={{ color: 'white' }}>Real amount(20% fees)</TableCell>
                    <TableCell sx={{ color: 'white' }}>Earned By</TableCell>
                    <TableCell sx={{ color: 'white' }}>Payment Date</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {mainState.payments.length>0?mainState.payments.map((order,index) => (
                    <TableRow key={index} hover sx={{ cursor: 'pointer' }}>
                      <TableCell>#PAY-{order.paymentID}</TableCell>
                      <TableCell>#ORD-{order.orderID}</TableCell>
                      <TableCell>${order.deliveryamount?.toFixed(2)}</TableCell>
                      <TableCell>{order.paymentMethod}</TableCell>
                      <TableCell>${order.realamount?.toFixed(2)}</TableCell>
                      <TableCell>{order.Company_name==='Admin'?"You":order.Company_name}</TableCell>
                      <TableCell>{order.paymentDate}</TableCell>
                    </TableRow>
                  )):<Typography variant='h6'> no current Payments found in this company</Typography>}
                </TableBody>
              </Table>
            </TableContainer>
          </Box>
        )}

       
        {viewMode === 'Reviews' && (
          <Box display={"flex"} flexWrap={"wrap"} alignItems={"center"} gap={"20px"}>
              {mainState.reviews.length>0?mainState.reviews.map((review)=>(
                  <Paper 
                  key={review.reviewID}
                  elevation={3}
                  sx={{
                    p: 4,
                    borderLeft: `3px solid ${theme.palette.primary.main}`,
                    borderRadius: '4px',
                    maxWidth: '320px'
                  }}
          >
            
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2,minWidth:100 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                <Star fontSize="small" sx={{ color: theme.palette.warning.main }} />
                <Typography variant="subtitle1" fontWeight="500">
                  Ratings of Order#{review.orderID}
                </Typography>
                 <Box >
                <Typography variant="h6" component="span" fontWeight="600">
                  {review.rating}
                </Typography>
                <Typography variant="body2" component="span" color="text.secondary">
                  /5
                </Typography>
              </Box>
              </Box>
             
            </Box>

            
            <Box sx={{ display: 'flex', justifyContent: 'center', mb: 3 }}>
              <Rating
                name="rating-filter"
                value={review.rating} 
                precision={0.5}
                readOnly
                size="large"
                icon={<Star fontSize="inherit" />}
                emptyIcon={<Star fontSize="inherit" />}
                sx={{
                  '& .MuiRating-icon': {
                    color: theme.palette.action.disabled
                  },
                  '& .MuiRating-iconFilled': {
                    color: theme.palette.warning.main
                  },
                  '& .MuiRating-iconHover': {
                    color: theme.palette.warning.dark
                  }
                }}
              />
            </Box>
            <Stack spacing={1}>
             <Typography variant="body2" component="span" color="text.secondary">Customer name:{review.customername}</Typography>
             <Typography variant="body2" component="span" color="text.secondary">Comments:{review.comments!=""?review.comments:"no comment added"}</Typography>
               <Typography variant="body2" component="span" color="text.secondary">Review Date:{new Date(review.reviewDate).toISOString().slice(0, 10)}</Typography>
                 <Typography variant="body2" component="span" color="text.secondary">Review time:{new Date(review.reviewDate).toTimeString().slice(0, 8)}</Typography>
            </Stack>

            
          </Paper>
          )
            
          ):<Typography variant='h6'> no current Review found in this company</Typography>}
          </Box>
         
         
        )}
      </Box>
    </Dialog>
  );
};

export default Store;
